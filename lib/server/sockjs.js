...
exports.attach = function(connectServer, createAgent, options) {
  var sjsServer;
  options || (options = {});
  sjsServer = new vertx.createSockJSServer(connectServer.server);
  stdout.print("Installing SockJS app at /channel");
  sjsServer.installApp({
    prefix: "/channel"
  }, function(sock) {
    var agent, close, data, docState, handleClose, handleMessage, handleOp, handleOpenCreateSnapshot, lastReceivedDoc, lastSentDoc, open, send;
    console.log("New SockJS session with id " + sock.id);
    data = {
      headers: sock.headers,
      remoteAddress: sock.address
    };
    agent = null;
    lastSentDoc = null;
    ...
    handleMessage = function(query) {
      var error, _name, _ref, _ref2, _ref3;
      console.log("Message from " + agent.sessionId + " for doc " + query.doc, query);
      error = null;
      ...
      if ((_ref2 = query.open) !== true && _ref2 !== false && _ref2 !== (void 0)) {
        error = "'open' must be true, false or missing";
      }
      if ((_ref3 = query.snapshot) !== null && _ref3 !== (void 0)) {
      ...
      docState[_name = query.doc] || (docState[_name] = {
        queue: syncQueue(function(query, callback) {
          ...
          } else if ((query.op != null) || (((_ref4 = query.meta) != null ? _ref4.path : void 0) != null)) {
    ...
    open = function(docName, version, callback) {
      ...
      docState[docName].listener = listener = function(opData) {
        ...
        if (opData.meta.source === agent.sessionId) return;
        opMsg = {
          doc: docName,
          op: opData.op,
          v: opData.v,
      ...
      return agent.listen(docName, version, listener, function(error, v) {
        if (error) delete docState[docName].listener;
    ...
    sock.dataHandler(function(buffer) {
      ...
      stdout.print("Got a message " + msg);
      return handleMessage(JSON.parse(msg));
    });
    stdout.print("Create Agent");
    return createAgent(data, function(error, agent_) {
      var buff;
      if (error) {
        return stdout.print("ERROR: " + error + "\n");
      } else {
        agent = agent_;
        buff = new vertx.Buffer(JSON.stringify({
